...
const Homepage = () => {
  ...
  const [selectedPaperForCitation, setSelectedPaperForCitation] =
    useState(null);
  ...
  const handlePaperTitleClick = (paperId) => {
    const id =
      typeof paperId === "object" && paperId !== null ? paperId.id : paperId;
  ...
  const filteredPapers = papers.filter((paper) => {
    const matchesSearch =
      ...
      (typeof paper.title === "object"
        ? (paper.title.text || paper.title.content || "")
            .toLowerCase()
      ...
      (paper.abstract &&
        (typeof paper.abstract === "object"
          ? (paper.abstract.text || paper.abstract.content || "")
              .toLowerCase()
              .includes(searchQuery.toLowerCase())
          : (paper.abstract || "")
              .toLowerCase()
      ...
      (paper.description &&
        (typeof paper.description === "object"
          ? (paper.description.text || paper.description.content || "")
              .toLowerCase()
              .includes(searchQuery.toLowerCase())
          : (paper.description || "")
              .toLowerCase()
    ...
    if (
      ...
      parseInt(paperYear) < parseInt(filters.yearRange.min)
    ) {
      return false;
    }
    if (
      filters.yearRange.max &&
      parseInt(paperYear) > parseInt(filters.yearRange.max)
    ) {
      return false;
    }
      ...
      <main className="main-content">
        <div className="content-container">
          ...
          <div className="title-section">
            <h2>
              <FiFileText size={24} /> Research Repository
            </h2>
            <p>
              Discover, explore, and engage with the latest computer and
                      ...
                      <div className="filter-section">
                        ...
                        {filters.sdgs.length > 0 && (
                          <div className="sdg-selection-summary">
                            {filters.sdgs.length} SDG
                            {filters.sdgs.length > 1 ? "s" : ""} selected
                            <button
                              onClick={clearSDGFilters}
                              className="sdg-clear-btn"
                            >
                              Clear SDGs
                            </button>
                          </div>
                    ...
                    <div className="paper-stats">
                      <div className="stats-left">
                        <span className="likes">
                          <FiThumbsUp size={16} />{" "}
                          {typeof paper.likes === "object"
                            ? paper.likes.id
                              ? String(paper.likes.id)
                              : "0"
                            : paper.likes || "0"}
                        </span>
                        <span className="comments">
                          <FiMessageCircle size={16} />{" "}
                          {Array.isArray(paper.comments)
                            ? paper.comments.length
                            : typeof paper.comments === "object"
                              ? paper.comments.id
                                ? String(paper.comments.id)
                                : "0"
                              : paper.comments || "0"}
                        </span>
                        <span className="citations">
                          <FiBookOpen size={16} /> {paper.citationCount || 0}
                        </span>
                      </div>
