...
app.use((req, res, next) => {
  try {
    ...
    if (req.query) Object.keys(req.query).forEach(k => { req.query[k] = sanitizeValue(req.query[k]); });
    if (req.params) Object.keys(req.params).forEach(k => { req.params[k] = sanitizeValue(req.params[k]); });
    if (req.body) Object.keys(req.body).forEach(k => { req.body[k] = sanitizeValue(req.body[k]); });
  } catch (err) {
